...
DOCTEXT = f"""
Usage:
  main [--weights=<w>...|--train_weights=<tw>] [--vector=<v>] [--embedding_dim=<ed>] [--num_seq_transformer=<nst>] [--num_heads_att=<nha>] [--batch_size=<bs>|(--fract=<f> --fract_dataset=<fd>)] [--epochs=<e>] [--lr=<lr>] [--torch_seed=<ts>] [--random_seed=<rs>] [--numpy_seed=<ns>]
  main -h | --help
...
Options:
  ...
  --num_heads_att=<nha>    Int. Number of self-attention heads of a Transformer Layer [default: {NUM_HEADS_ATT}].
...
def parse_args(argv):
    ...
    num_heads_att = int(opts['--num_heads_att']) 
    ...
    args = (experiment_name, weights, vector, train_weights, embedding_dim, num_seq_transformer, num_heads_att, batch_size, fract, fract_dataset, epochs, lr, torch_seed, random_seed, numpy_seed)
